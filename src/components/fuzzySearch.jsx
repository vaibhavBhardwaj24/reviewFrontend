...
import { FuzzySearch } from "./ui/placeholders-and-vanish-input";
...
function FuzzySearchComponent() {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (resultsRef.current && !resultsRef.current.contains(event.target)) {
        setResults([]); // Clear results if clicked outside
      }
    };
  ...
  return (
    <div className="flex justify-center">
      <FuzzySearch onChange={handleSearch} placeholders={placeholders} />
      <ul
        className="absolute w-1/3 translate-y-12 z-20 bg-white cursor-default font-light rounded-b-lg text-black "
        ref={resultsRef}
      >
        {results.length > 0 ? (
          results.map((device) => (
            <li
              key={device.id}
              className="gap-2  overflow-hidden px-3 flex items-center hover:bg-black/10 duration-300 border-b-[1px] border-black/50"
            >
              <img
                src={device.img}
                alt={device.name}
              ...
              <div className="flex">
                <p className="text-2xl">
                  {device.description.split(" ")[0]} {device.name}
                </p>
              </div>
            </li>
          ))
